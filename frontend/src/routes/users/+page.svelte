...
<div class="flex flex-col w-full h-full items-center justify-start">
	<h1 class="text-2xl m-4 font-bold">Leaderboard</h1>
	<div class="w-3/4 overflow-x-scroll mb-20">
		{#each users as user, id}
			<button on:click={() => goto(`/user/${user.username}`)} transition:fly={{delay: id * 50, y:50}} class="flex bg-slate-800 rounded-md w-full p-3 my-1 justify-between items-center text-center">
